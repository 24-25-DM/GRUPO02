    ...
    const fetchVehiculos = async () => {
        try {
            ...
            if (state.isConnected) {
                // Obtener datos desde AWS
                const response = await axios.get('https://6779-104-199-173-152.ngrok-free.app/vehiculos');
                const vehiclesFromAWS = response.data.map((vehicle, index) => ({
                    id: index, // Asignar un id único si no existe
                    ...vehicle,
                    image: vehicle.imagen ? `data:image/jpeg;base64,${Buffer.from(vehicle.imagen.data).toString('base64')}` : null
                }));
                setVehiculos(vehiclesFromAWS);
                console.log("Lista de vehículos desde AWS", vehiclesFromAWS);
            } else {
                // Obtener datos desde la base de datos local
                const allRows = await db.getAllAsync("SELECT * FROM vehicles");
                setVehiculos(allRows);
                console.log("Lista de vehículos desde la base de datos local", allRows);
            }
        } catch (error) {
