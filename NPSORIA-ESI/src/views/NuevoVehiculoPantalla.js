...
const NuevoVehiculoPantalla = ({ navigation }) => {
    ...
    const handleGuardar = () => {
        ...
        /*       En la pantalla de infreso de vehiculos, valide que la placa tenga las siguentes caracteristicas
              *Formato "XXX-NNNN" donde X es letra y N es numero
              *No iniciar con letras: D, F,Q,Ñ
              *NO TENER LETRAS NI NUMERO CONSECUTIVOS
              *No incluir el numero 0 si la placa contiene una O
              Si no se cumple, muestre un mensaje indicando cuales no se cumplen */
        ...
        if (aux[0] === 'D' || aux[0] === 'F' || aux[0] === 'Q' || aux[0] === 'Ñ') {
            Alert.alert("Error", "La Placa no puede comenzar con D,F,Q,Ñ");
            return;
        }

        if (aux.includes('O') && aux.includes('0')) {
            Alert.alert("Error", "No incluir el numero 0 si la placa contiene una O");
            return;
        }
