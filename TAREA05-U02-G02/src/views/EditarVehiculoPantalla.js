...
const EditarVehiculoPantalla = ({ navigation, route }) => {
    ...
    useEffect(() => {
        // Función asíncrona para cargar el vehículo desde la base de datos
        const fetchVehiculo = async () => {
            try {
                const vehiculoEncontrado = await obtenerVehiculoPorId(db,id);
                if (vehiculoEncontrado) {
                    setPlaca(vehiculoEncontrado.placa);
                ...
                console.log("Error while loading vehicle:", error);
            }
        };

        fetchVehiculo(); // Llamar la función
    }, [id, db, navigation]); // Ejecutar solo cuando `id` cambie
    ...
    const handleEditVehicle = async () => {
        ...
        await editarVehiculo(db,id,updatedVehiculo);
