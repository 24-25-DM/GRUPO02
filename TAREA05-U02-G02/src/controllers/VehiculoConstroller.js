...
export const agregarVehiculo = async (db, vehiculo) => {
    try {
        await insertVehicle(db, vehiculo.placa, vehiculo.marca, vehiculo.fecFabricacion, vehiculo.color, vehiculo.costo, vehiculo.activo);
...
// Editar un vehículo
export const editarVehiculo = async (db, id, datosActualizados) => {
    try {
        await updateVehicle(db, id, datosActualizados);
